...
//         <Button
//           asChild
//           className="bg-yellow-100 text-yellow-800 hover:bg-green-200 px-4 py-2 rounded font-medium"
//         >
//           <Link to="/profile">Profile Settings</Link>
//         </Button>

//         {/* 💬 Chat Button — moved outside the form */}
//         {form.doctorName && (
//           <Button
//             as={Link}
//             to={`/chat/${form.doctorName}/${user._id}`}
//             type="button"
//             className="bg-green-600 text-white hover:bg-green-700 px-4 py-2 rounded font-medium"
//           >
//             💬 Chat with Dr. {form.doctorName}
//           </Button>
//         )}
//       </div>
...
export default function BookAppointment() {
  ...
  const [form, setForm] = useState({
    patientName: user?.name || '',
    doctorName: '', // now stores doctor ID
    date: '',
    time: '',
    reason: '',
  });
  ...
  const fetchDoctors = async () => {
    try {
      const res = await api.get('/doctors'); // assumes GET /doctors returns [{ _id, name }]
      setDoctors(res.data);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 right-6 flex items-center gap-4 z-20">
        <Link to="/appointments/upcoming" title="View Upcoming Appointments">
          <FiBell
            size={34}
            className={`cursor-pointer ${
              hasSoonAppointment ? 'text-red-600 animate-bounce' : 'text-green-700'
            }`}
          />
        </Link>
      ...
      <div className="flex flex-col gap-4 pt-4 pr-6">
        ...
        {/* 💬 Chat Button — uses real doctor ID */}
