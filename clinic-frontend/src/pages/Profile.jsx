...
export default function Profile() {
  ...
  // Compute backend URL from VITE_API_URL (fallback to localhost for dev)
  const BACKEND_URL =
    import.meta.env.VITE_API_URL?.replace(/\/$/, '') || 'http://localhost:5000'

  // Fetch user profile (doctor or patient) on mount / role change
  useEffect(() => {
    if (!user?.role) return

    const endpoint =
      user.role === 'doctor' ? '/doctors/me' : '/patients/me'
